...
const textArray = ["Web Developer⌨️🖱️📝", "Creative Coder⌨️🖱️📝", "UI / UX Designer⌨️🖱️📝"];
let index = 0;
let charIndex = 0;
...
const tronForm = document.querySelector(".tron-form");

if (tronForm) {
  tronForm.addEventListener("submit", (e) => {
    e.preventDefault(); 
    clearErrors();

    let valid = true;

    tronForm.querySelectorAll("input, textarea").forEach((field) => {
...
function clearErrors() {
  document.querySelectorAll(".tron-error").forEach((el) => el.remove());
}
